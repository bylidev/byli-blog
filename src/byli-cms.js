...
const imagesDir = './assets/blog/images';
const entryPath = './assets/blog';
const metadataFilePath = './assets/blog/manifest.json';
...
function createManifest(fileNames, metadataFilePath) {
  ...
  fileNames.forEach(fileName => {
    ...
    validateJson(data);
    data.thumb = imagesDir.replace('./','/') +'/thumb_' + data.thumb;
    const uuid = uuidv4();
    data.md = entryPath.replace('.','') + '/' + uuid;
    ...
    const cleanMdContent = matter(fileContent).content.replace(/\(\.\/images/g, '('+imagesDir.replace('./', '/'));
